...
import { mergeFunctionCovs, mergeProcessCovs, mergeScriptCovs } from "../lib";
...
function loadFixture(name) {
    const content = fs.readFileSync(path.resolve(FIXTURES_DIR, `${name}.json`), { encoding: "UTF-8" });
        ...
        for (const bench of getBenches()) {
            ...
            function testBench() {
                return __awaiter(this, void 0, void 0, function* () {
                    ...
                    const inputFileNames = yield fs.promises.readdir(bench);
                    const inputPromises = [];
                    ...
                    const inputs = yield Promise.all(inputPromises);
                    const expectedPath = path.join(BENCHES_DIR, `${name}.json`);
                    const expectedContent = yield fs.promises.readFile(expectedPath, { encoding: "UTF-8" });
