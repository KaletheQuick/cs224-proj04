...
export function populateSWTemplate({ cacheId, cleanupOutdatedCaches, clientsClaim, directoryIndex, disableDevLogs, ignoreURLParametersMatching, importScripts, manifestEntries = [], navigateFallback, navigateFallbackDenylist, navigateFallbackAllowlist, navigationPreload, offlineGoogleAnalytics, runtimeCaching = [], skipWaiting, }) {
    ...
    try {
        ...
        throw new Error(`${errors['populating-sw-tmpl-failed']} '${error instanceof Error && error.message ? error.message : ''}'`);
    }
}
