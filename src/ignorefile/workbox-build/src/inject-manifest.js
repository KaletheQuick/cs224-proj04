 ...
 */
export function injectManifest(config) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const { count, size, manifestEntries, warnings } = yield getFileManifestEntries(options);
        let swFileContents;
        try {
            swFileContents = yield fse.readFile(options.swSrc, 'utf8');
        }
        catch (error) {
        ...
        if (srcPath && destPath) {
            const originalMap = (yield fse.readJSON(srcPath, {
                encoding: 'utf8',
            }));
            const { map, source } = yield replaceAndUpdateSourceMap({
                originalMap,
        ...
        for (const [file, contents] of Object.entries(filesToWrite)) {
            try {
                yield fse.mkdirp(upath.dirname(file));
            }
            catch (error) {
                throw new Error(errors['unable-to-make-sw-directory'] +
                    ` '${error instanceof Error && error.message ? error.message : ''}'`);
            }
            yield fse.writeFile(file, contents);
        }
        return {
            count,
            size,
