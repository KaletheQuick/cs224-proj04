import { DRAFT_STATE, isDraftable, NOTHING, each, has, freeze, isDraft, set, getPlugin, die, revokeScope, isFrozen, shallowCopy } from "../internal";
...
function finalize(rootScope, value, path) {
    ...
    if (!state.finalized_) {
        ...
        state.type_ === 4 /* ProxyType.ES5Object */ || state.type_ === 5 /* ProxyType.ES5Array */
        ...
        let isSet = false;
        if (state.type_ === 3 /* ProxyType.Set */) {
...
function finalizeProperty(rootScope, parentState, targetObject, prop, childValue, rootPath, targetIsSet) {
    ...
    if (isDraft(childValue)) {
        const path = rootPath &&
            parentState &&
            parentState.type_ !== 3 /* ProxyType.Set */ && // Set objects are atomic since they have no keys.
